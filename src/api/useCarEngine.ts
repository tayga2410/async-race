...
const useCarEngine = () => {
  ...
  const switchToDriveMode = async (carId: number) => {
    try {
      ...
      if (!responseData.success) {
        setIsDriving(false);
        console.log(`Car with ID ${carId} encountered an issue and stopped.`);
      }
    } catch (error) {
  ...
  return {
    isDriving, carEngineInfo, startEngine, stopEngine, switchToDriveMode, getElapsedTimeInSeconds,
  };
};
