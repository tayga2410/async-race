...
const Garage: React.FC = () => {
  ...
  const handleSaveCar = async () => {
    ...
    try {
      ...
      const updatedCarList = carList.map((car) => (car.id === updatedCar.id ? { ...car, name: updatedCar.name, color: updatedCar.color } : car));
      ...
      <ul>
        {carList.map((car) => (
          <li className="car-list__item" key={car.id}>
            ...
            <p className='car-list__name'>{car.name}</p>
            <div className={`car-icon-container ${engineStatus[car.id] ? 'driving' : 'stopped'}`}>
              <CarIcon color={car.color} className={`car-list__icon ${engineStatus[car.id] ? 'driving' : 'stopped'}`} />
            </div>
            {engineStatus[car.id] && (
            <p>
              Elapsed Time:
              {' '}
              {Math.round(getElapsedTimeInSeconds(car.id))}
              {' '}
              seconds
            </p>
            )}
            {engineStatus[car.id] === false && (
            <p>Car stopped due to engine issue.</p>
            )}
          </li>
        ))}
      </ul>
